...
class SourceBlock {
  ...
  constructor(
    ...
    public transleteWords: HTMLDivElement,
  ) {
    this.words = words;
    this.round = round;
    this.wordCards = [];
    ...
    this.wordForSource = new WordsForSourceBlock(
      this.round,
      this.continueBtn,
      this.wordCards,
      this.checkBtn,
  ...
  createCorrectCards(): void {
    const resultRound: NodeListOf<HTMLDivElement> = document.querySelectorAll(
      ".play-field_result-round",
    );
    const resultCards = resultRound[this.round - 1];
    const sourceCards: HTMLDivElement | null =
      document.querySelector(".play-field_source");
