...
class FormValidation extends ResultValidation {
  ...
  setupValidation(): void {
    ...
    const surnameInput: HTMLInputElement | null =
      document.querySelector("input#surname");
    const errorFirstName: HTMLDivElement | null =
      document.querySelector(".first-name_error");
    const errorSurname: HTMLDivElement | null =
      document.querySelector(".surname_error");
    if (firstNameInput) {
      firstNameInput.addEventListener("input", () => {
        ...
        this.loginButton.setDisabled(
          !this.updateValidity(this.isValidFirstName, this.isValidSurname),
        );
